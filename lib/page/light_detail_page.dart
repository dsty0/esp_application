...
class LightDetailPage extends StatefulWidget {
  ...
  const LightDetailPage({Key? key, required this.tokenId}) : super(key: key);
...
class _LightDetailPageState extends State<LightDetailPage> {
  ...
  List<Map<String, dynamic>> get filteredData {
    final entries = allData.entries.map((e) {
      ...
      return {
        'timestamp': ts,
        'cahaya': (e.value['cahaya'] ?? 0).toDouble(),
      };
    }).where((e) => e['timestamp'] != null).toList();
  ...
  Future<void> exportCSV() async {
    ...
    for (var item in data) {
      ...
      final cahaya = item['cahaya'];
      csv.writeln("${DateFormat('yyyy-MM-dd HH:mm:ss').format(ts)},$cahaya");
    }

    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/cahaya_data.csv');
    await file.writeAsString(csv.toString());

    Share.shareXFiles([XFile(file.path)], text: 'Cahaya Data CSV');
  }
                ...
                lineBarsData: [
                  LineChartBarData(
                    spots: List.generate(reversedData.length, (i) {
                      final cahaya = reversedData[i]['cahaya'];
                      return FlSpot(i.toDouble(), cahaya);
                    }),
                    isCurved: true,
