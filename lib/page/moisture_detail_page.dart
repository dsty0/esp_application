...
class MoistureDetailPage extends StatefulWidget {
  final String tokenId;

  const MoistureDetailPage({Key? key, required this.tokenId}) : super(key: key);

  @override
  _MoistureDetailPageState createState() => _MoistureDetailPageState();
}

class _MoistureDetailPageState extends State<MoistureDetailPage> {
  ...
  Future<void> exportCSV() async {
    ...
    for (var item in data) {
      ...
      final soil = item['soil'];
      csv.writeln("${DateFormat('yyyy-MM-dd HH:mm:ss').format(ts)},$soil");
    }

    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/soil_data.csv');
    await file.writeAsString(csv.toString());

    Share.shareXFiles([XFile(file.path)], text: 'Moisture Data CSV');
  }
