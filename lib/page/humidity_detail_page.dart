...
class HumidityDetailPage extends StatefulWidget {
  final String tokenId;

  const HumidityDetailPage({Key? key, required this.tokenId}) : super(key: key);

  @override
  _HumidityDetailPageState createState() => _HumidityDetailPageState();
}

class _HumidityDetailPageState extends State<HumidityDetailPage> {
  ...
  List<Map<String, dynamic>> get filteredData {
    final entries = allData.entries.map((e) {
      ...
      return {
        'timestamp': ts,
        'kelembapan': (e.value['kelembapan'] ?? 0).toDouble(),
      };
    }).where((e) => e['timestamp'] != null).toList();
  ...
  Future<void> exportCSV() async {
    ...
    for (var item in data) {
      ...
      final kelembapan = item['kelembapan'];
      csv.writeln("${DateFormat('yyyy-MM-dd HH:mm:ss').format(ts)},$kelembapan");
    }

    final dir = await getTemporaryDirectory();
    final file = File('${dir.path}/kelembapan_data.csv');
    await file.writeAsString(csv.toString());

    Share.shareXFiles([XFile(file.path)], text: 'Kelembaban Data CSV');
  }
                ...
                lineBarsData: [
                  LineChartBarData(
                    spots: List.generate(reversedData.length, (i) {
                      final kelembapan = reversedData[i]['kelembapan'];
                      return FlSpot(i.toDouble(), kelembapan);
                    }),
                    isCurved: true,
